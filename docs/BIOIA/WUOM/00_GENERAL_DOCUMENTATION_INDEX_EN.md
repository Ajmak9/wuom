# BIOIA / WUOM — General Documentation Index (EN)

**Status:** public navigation index  
**Version:** 0.1  
**Date:** 1 August 2026  
**Language:** EN  
**Canonical platform:** GitHub

> **Unity of architecture, diversity of doors.**

This index is the guided English entry point to the public BIOIA / WUOM documentation.

It does not duplicate, move, reinterpret or replace the canonical documents.  
It organizes access to them according to the functional structure of the public website.

---

## Access logic

| Access point | Function |
|---|---|
| **Public website** | Guided public entry through the six web routes |
| **This index** | Structured English guide to the canonical documentation |
| **Repository root** | Complete GitHub archive |
| **Root README** | General repository orientation |

- [Open the public BIOIA / WUOM website](https://web.bioiawuom.org/?lang=en)
- [Open the complete GitHub repository](https://github.com/Ajmak9/wuom)
- [Open the root README](https://github.com/Ajmak9/wuom/blob/main/README.md)

---

# 00 · Orientation and Core

Public function: establish the purpose, limits, orientation and derivation rules of BIOIA / WUOM.

- [BIOIA / WUOM — Repository Manifest](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/00_MANIFEST.md)
- [BIOIA / WUOM — Minimal Orientation Repository](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/README.md)
- [Public website — Start](https://web.bioiawuom.org/?lang=en)
- [Public website — Origin](https://web.bioiawuom.org/origen?lang=en)

Core operational sequence:

```text
territory
→ human experience
→ AI-assisted reading
→ deliberation
→ situated decision
→ action
→ verification
→ territorial return
```

> **Orientation precedes action.**  
> **Without return, there is extraction.**

---

# 01 · Architecture

Public function: situate every stable element within the complete BIOIA / WUOM architecture.

- [Public website — Architecture](https://web.bioiawuom.org/arquitectura?lang=en)
- [BIOIA / WUOM Master Plan — Definition and Scope](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/01_EVOLUTIONARY_LINE/05_ARCHITECTURAL_DEPLOYMENT/04_BIOIA_WUOM_MASTER_PLAN/01_DEFINITION_AND_SCOPE_OF_THE_BIOIA_WUOM_MASTER_PLAN_EN_v1.0.md)
- [Architectural Deployment — Complete folder](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/01_EVOLUTIONARY_LINE/05_ARCHITECTURAL_DEPLOYMENT)
- [BIOIA / WUOM Master Plan — Folder](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/01_EVOLUTIONARY_LINE/05_ARCHITECTURAL_DEPLOYMENT/04_BIOIA_WUOM_MASTER_PLAN)

Architecture does not replace the core.  
It makes the relationships between core, principles, trees, interfaces, applications and return traceable.

---

# 02 · Reading Organs

Public function: organize complementary perspectives for reading living systems before intervention.

- [Public website — Reading Organs](https://web.bioiawuom.org/organos?lang=en)
- [System of BIOIA Trees v1.0 — Main document](https://github.com/Ajmak9/wuom/blob/main/docs/SYSTEM_OF_BIOIA_TREES_v1.0/00_README.md)
- [System of BIOIA Trees v1.0 — Complete folder](https://github.com/Ajmak9/wuom/tree/main/docs/SYSTEM_OF_BIOIA_TREES_v1.0)

The system includes seven initial operational perspectives:

1. Horizon Tree
2. Wind Tree
3. Fog Tree
4. Fire Tree
5. Living Tree
6. Energy Tree
7. Time Tree

The trees are reading organs.  
They are not products and do not replace scientific, territorial or human knowledge.

---

# 03 · Applications and Interfaces

Public function: translate the architecture into situated, traceable and limited applications.

- [Public website — Applications](https://web.bioiawuom.org/aplicaciones?lang=en)
- [BIOIA / WUOM — Applications Index](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/00_APPLICATIONS_INDEX.md)
- [Applications — Complete folder](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/04_APPLICATIONS)
- [Territorial GIS Application — Main document](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/01_TERRITORIAL_GIS_APPLICATION/README.md)
- [Node Bee v0.1](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/NODE_BEE_v0.1.md)

Application rule:

```text
field
+ layers
+ relations
+ tension
+ reading
+ orientation
+ decision
+ return
```

Applications do not redefine the root architecture.  
They translate reading into situated domains.

---

# 04 · Territorial Cases

Public function: document situated observation, pressures, actors, uncertainty, custody and return without imposing closed causality.

- [Applications Index — Active applications and territorial cases](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/00_APPLICATIONS_INDEX.md)
- [Applications — Territorial documentation folder](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/04_APPLICATIONS)
- [Territorial GIS Application](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/04_APPLICATIONS/01_TERRITORIAL_GIS_APPLICATION)

Territorial cases remain open when evidence is incomplete.  
Observation, verification and custody precede conclusion.

---

# 05 · Situated Genealogy

Public function: preserve the traceable emergence of the architecture from artistic origin to situated territorial verification.

- [Situated Genealogy — Map of Connections](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/01_EVOLUTIONARY_LINE/05_ARCHITECTURAL_DEPLOYMENT/06_ECOSYSTEMIC_GOVERNANCE_METAARCHITECTURE_EN/07_SITUATED_GENEALOGY_EN/02_SITUATED_GENEALOGY_MAP.md)
- [Situated Genealogy — Complete folder](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/01_EVOLUTIONARY_LINE/05_ARCHITECTURAL_DEPLOYMENT/06_ECOSYSTEMIC_GOVERNANCE_METAARCHITECTURE_EN/07_SITUATED_GENEALOGY_EN)
- [Ecosystemic Governance Meta-architecture — Folder](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM/01_EVOLUTIONARY_LINE/05_ARCHITECTURAL_DEPLOYMENT/06_ECOSYSTEMIC_GOVERNANCE_METAARCHITECTURE_EN)

Genealogical sequence:

```text
artistic question
→ EVA W
→ WUOM
→ BIOIA
→ territorial verification
→ return
```

The origin remains.  
The architecture opens.

---

# 06 · WUOM

Public function: provide operational geometries and instruments for organizing complexity before acting.

- [WUOM — Root README](https://github.com/Ajmak9/wuom/blob/main/README.md)
- [Documentation archive](https://github.com/Ajmak9/wuom/tree/main/docs)
- [Images archive](https://github.com/Ajmak9/wuom/tree/main/images)
- [Templates archive](https://github.com/Ajmak9/wuom/tree/main/templates)
- [WUOM public access](https://wuom.carrd.co)
- [WUOM / BIOIA tools on Gumroad](https://roblesrionegro.gumroad.com)

WUOM provides access and operational reading.  
BIOIA provides the situated field in which that reading acquires meaning and responsibility.

---

## Recent documentary readings

- [38 · DISORIENTED](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/31_DIEGO_MEMORANDUM/38_DISORIENTED_EN.pdf)
- [39 · THE TRILOGY OF ACCUMULATIVE MISGOVERNANCE](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/31_DIEGO_MEMORANDUM/39_TRILOGY_OF_ACCUMULATIVE_MISGOVERNANCE_EN.pdf)
- [41 · THE ONE COLLECTIVE · THE ONE IRRESPONSIBLE](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/31_DIEGO_MEMORANDUM/41_BIOIA_WUOM_THE_ONE_COLLECTIVE_THE_ONE_IRRESPONSIBLE_EN.pdf)
- [42 · THE WRONG AXIS](https://github.com/Ajmak9/wuom/blob/main/docs/BIOIA/WUOM/04_APPLICATIONS/31_DIEGO_MEMORANDUM/42_BIOIA_WUOM_THE_WRONG_AXIS_EN.pdf)
- [43 · BIOIA COMPLETE ORGANISM](04_APPLICATIONS/31_DIEGO_MEMORANDUM/43_BIOIA_COMPLET_ORGANISM_EN.pdf)
- [44 · THE CLASSIFICATION ERROR](04_APPLICATIONS/31_DIEGO_MEMORANDUM/44_THE_CLASSIFICATION_ERROR_EN.pdf)
- [45 · THE VISIBILITY THRESHOLD](04_APPLICATIONS/31_DIEGO_MEMORANDUM/45_THE_THRESHOLD_OF_VISIBILITY_EN.pdf)
- [46 · VISIBLE, BUT NOT OPERATIONAL](04_APPLICATIONS/31_DIEGO_MEMORANDUM/46_VISIBLE_BUT_NOT_OPERATIONAL_EN.pdf)
- [47 · THE CHROMATIC TRANSITION](04_APPLICATIONS/31_DIEGO_MEMORANDUM/47_THE_CHROMATIC_TRANSITION_EN.pdf)
- [48 · THE ACCELERATION OF POSTPONEMENT](04_APPLICATIONS/31_DIEGO_MEMORANDUM/48_THE_ACCELERATION_OF_POSTPONEMENT_EN.pdf)
- [49 · THE DOUBLE DIRECTION OF PRESSURE](04_APPLICATIONS/31_DIEGO_MEMORANDUM/49_THE_DOUBLE_DIRECTION_EN.pdf)
- [50 · OUTSIDE THE AXIS](04_APPLICATIONS/31_DIEGO_MEMORANDUM/50_OUTSIDE_THE_AXIS_EN.pdf)
  
---

### [51 · THE ORDER OF PRIORITIES](../../51_THE_ORDER_OF_PRIORITIES_EN.pdf)
[Zenodo · DOI 10.5281/zenodo.21904155](https://doi.org/10.5281/zenodo.21904155)
---

### 52 · THE EVIDENT

Examines the difference between intelligence, capability, and responsibility in the use of AI. It argues that calculation, search, or parts of a decision can be delegated, but responsibility for the consequences cannot, and returns the analysis to the territory through the relationship:

**capability → decision → consequence → territory**

[PDF · 52_THE_EVIDENT_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/52_THE_EVIDENT_EN.pdf)

---

### 53 · SUPERINTELLIGENCE FOR ALL · TERRITORY FOR NONE

Examines the promise of widely distributed artificial intelligence against the material and territorial conditions that sustain it. It argues that democratizing access does not yet mean democratizing power, infrastructure, or decision-making capacity, and that the equalizing promise remains incomplete while questions of control, transparency, responsibility, concentration, and territorial return remain unresolved.

[PDF · 53_SUPERINTELLIGENCE_FOR_ALL_TERRITORY_FOR_NONE_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/53_SUPERINTELLIGENCE_FOR_ALL_TERRITORY_FOR_NONE_EN.pdf)

---

## 54 · WHERE IS THE TRANSITION?

If the old remains and the new is added.
Analyzes the gap between the declared transition and effective material change. It distinguishes substitution from superposition, shows how new infrastructures can be added without displacing existing ones, and introduces the temporal variable: more time to reduce pressure and more speed to add capacity. It proposes the hypothesis of accelerated superposition and argues that transition should be verified through material flows, pressure reduction, and the actual territorial balance.

[PDF · 54_WHERE_IS_THE_TRANSITION_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/54_WHERE_IS_THE_TRANSITION_EN.pdf)

---

## 55 · THE SPEED OF ADAPTATION

[PDF · 55_THE_SPEED_OF_ADAPTATION_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/55_THE_SPEED_OF_ADAPTATION_EN.pdf)

---

## 56 · THE TIME OF RETURN

[PDF · 56_THE_TIME_OF_RETURN_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/56_THE_TIME_OF_RETURN_EN.pdf)

---

## 59 · DIEGO MEMORANDUM

17 July — 17 August 2026.
Presents a situated observation of the thermal signal across health, production, water, biodiversity, fires, work, transport, energy, infrastructure, services, and economic activity. It documents cross-sector impacts, operational capacity limits, and territorial costs, including damage that remains outside available economic accounting.

[PDF · 59_DIEGO_MEMORANDUM_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/59_DIEGO_MEMORANDUM_EN.pdf)

---

## 60 · SYNTHESIS BY SUPERPOSITION 0.1

Proposes a joint reading of territory, time, capacity, infrastructure, adaptation, responsibility, and artificial intelligence. It argues that territory operates in the present while decisions and objectives can be postponed, and verifies transformation through what decreased, remained, or was added, how pressure changed, and what return materially reached the territory.

[PDF · 60_SYNTHESIS_BY_SUPERPOSITION_0.1_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/60_SYNTHESIS_BY_SUPERPOSITION_0.1_EN.pdf)

---

## ISOTYPIC MENU OF THE CURRENT ORDER

[PDF · ISOTYPIC_MENU_OF_THE_CURRENT_ORDER_EN.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/ISOTYPIC_MENU_OF_THE_CURRENT_ORDER_EN.pdf)

[Zenodo · DOI 10.5281/zenodo.22081813](https://doi.org/10.5281/zenodo.22081813)

---

## BLACK DESSERT MENU · ISOTYPES

[PDF · BLACK_DESSERT_MENU_ISOTYPES.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/BLACK_DESSERT_MENU_ISOTYPES.pdf)

[Zenodo · DOI 10.5281/zenodo.22097532](https://doi.org/10.5281/zenodo.22097532)

---

## CURRENT INVERTED ORDER · THE LOOP

[PDF · CURRENT_INVERTED_ORDER_THE_LOOP.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/CURRENT_INVERTED_ORDER_THE_LOOP.pdf)

[Zenodo · DOI 10.5281/zenodo.22099681](https://doi.org/10.5281/zenodo.22099681)

---

## COFFEE OR INFUSION · ISOTYPES OF THE INVERTED ORDER

[PDF · COFFEE_OR_INFUSION_ISOTYPES_OF_THE_INVERTED_ORDER.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/COFFEE_OR_INFUSION_ISOTYPES_OF_THE_INVERTED_ORDER.pdf)

[Zenodo · DOI 10.5281/zenodo.22100417](https://doi.org/10.5281/zenodo.22100417)

---

## THE WHOLE COIN

Examines how the expansion of networks and AI can advance while human and territorial margins decline. It frames this as a loss of correction capacity over time and places responsibility for reorientation in the human collective.

[PDF · THE_WHOLE_COIN_Abstract_acceleration_delegation_material_degradation_loss of margin.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/THE_WHOLE_COIN_Abstract_acceleration_delegation_material_degradation_loss%20of%20margin.pdf)

[Zenodo · DOI 10.5281/zenodo.22111164](https://doi.org/10.5281/zenodo.22111164)

---

## WHO IS IN CHARGE? · MANY HANDS / NO HAND

[PDF · WHO_IS_IN_CHARGE_MANY_HANDS_NO_HAND.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/WHO_IS_IN_CHARGE_MANY_HANDS_NO_HAND.pdf)

---

## HOW MUCH IS WHAT MAKES PRICE POSSIBLE WORTH?

Examines the limits of economic valuation and the material conditions that make price, markets and continuity possible.

[PDF · HOW_MUCH_IS_WHAT_MAKES_PRICE_POSSIBLE_WORTH.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/HOW_MUCH_IS_WHAT_MAKES_PRICE_POSSIBLE_WORTH.pdf)

[Zenodo · DOI 10.5281/zenodo.22130001](https://doi.org/10.5281/zenodo.22130001)

---

## DECOUPLING BETWEEN THREAT AND PROTECTION

Examines the gap between material threats and political protection, focusing on how temperature, water and territorial conditions shape security, budgets, infrastructure and Europe’s capacity to orient its priorities.

[PDF · DECOUPLING_BETWEEN_THREAT_AND_PROTECTION.pdf](04_APPLICATIONS/31_DIEGO_MEMORANDUM/DECOUPLING_BETWEEN_THREAT_AND_PROTECTION.pdf)

[Zenodo · DOI 10.5281/zenodo.22149469](https://doi.org/10.5281/zenodo.22149469)

---

# 07 · Versions and Status

Public function: preserve traceability, current state and the distinction between stable, open, observed and developing materials.

## Current public state

- Public website: **trilingual and operational**
- CAT documentation: **Notion CAT**
- ES documentation: **Notion ES**
- EN documentation: **GitHub EN**
- Public visual system: **CAT · ES · EN**
- Complete English archive: **GitHub repository**

## Repository traceability

- [Repository commits](https://github.com/Ajmak9/wuom/commits/main)
- [Repository root](https://github.com/Ajmak9/wuom)
- [BIOIA / WUOM documentation root](https://github.com/Ajmak9/wuom/tree/main/docs/BIOIA/WUOM)

Status terms used across the documentation:

| Status | Meaning |
|---|---|
| **Documented** | Stable public documentation exists |
| **Open** | Active field without forced closure |
| **In observation** | Requires further evidence or maturity |
| **In progress** | Public development continues |
| **Reserved** | Not opened in the current version |

---

# Complete Archive

This guided index is not the complete archive.

For the full repository, including documentation, images, templates, examples and historical structure:

> [Open the complete Ajmak9/wuom repository](https://github.com/Ajmak9/wuom)

---

# Contact

For documentation enquiries, technical issues and collaborations:

✉️ [wuom.system@gmail.com](mailto:wuom.system@gmail.com)

---

# License and Custody

Public documentation is shared under **CC BY-NC-SA 4.0**, unless a specific file states otherwise.

Any derivation must preserve:

1. provenance and context;
2. limits and uncertainty;
3. human responsibility;
4. territorial verification;
5. custody of data and knowledge;
6. identifiable return to territory or community.

> **Without stewardship, there is no publication.**  
> **Without return, there is no orientation.**

---

**BIOIA / WUOM**  
Art · Orientation · Territory  
Reading · Criterion · Return